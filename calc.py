...
Версия 10.0 — РАСШИРЕННЫЕ КАЛЬКУЛЯТОРЫ
"""
...
# ===================================================================
# 5. КАЛЬКУЛЯТОРЫ (РАСШИРЕННЫЕ)
# ===================================================================

# ----- 5.1. ФИНАНСОВЫЙ ОТДЕЛ (CEO) - 8 калькуляторов -----
CEO_CALCS = {
    "net_profit": {
        "name": "Чистая прибыль",
        "inputs": ["Выручка (руб)", "Расходы (руб)", "Налоги (руб)"],
        "formula": "a - b - c",
        "interpretation": "Чистая прибыль — главный показатель здоровья бизнеса. Это деньги, которые остаются после всех расходов.",
    ...
    "roe": {
        ...
        "formula": "(a / b) * 100",
        "interpretation": "ROE показывает, сколько прибыли приносит каждый рубль вложенных средств собственника.",
    ...
    "roa": {
        ...
        "formula": "(a / b) * 100",
        "interpretation": "ROA показывает, как эффективно используются все активы компании.",
    ...
    "cash_flow": {
        ...
        "formula": "a - b",
        "interpretation": "Положительный поток — бизнес генерирует деньги. Отрицательный — сжигает.",
    ...
    "ebitda": {
        ...
        "formula": "a + b + c + d",
        "interpretation": "EBITDA показывает операционную эффективность бизнеса без учёта финансовых и налоговых факторов.",
        "advice": "1. Оптимизируйте операционные расходы\n2. Повысьте эффективность процессов\n3. Внедрите управленческий учёт",
    ...
    "roi": {
        ...
        "formula": "(a / b) * 100",
        "interpretation": "ROI показывает, сколько процентов приносят вложенные инвестиции.",
        "advice": "1. Рассчитывайте ROI до начала инвестиций\n2. Сравнивайте ROI разных проектов\n3. Инвестируйте только в проекты с ROI выше 30%",
        "problems": "❌ Инвестиции без расчёта окупаемости\n❌ Нет системы оценки проектов\n❌ Деньги вложены неэффективно"
    }
}

# ----- 5.2. МАРКЕТИНГ - 6 калькуляторов -----
MARKETING_CALCS = {
    "cac": {
        "name": "Стоимость привлечения клиента (CAC)",
        "inputs": ["Расходы на маркетинг (руб)", "Количество новых клиентов"],
        "formula": "a / b",
        "interpretation": "CAC — сколько денег вы тратите на привлечение одного клиента.",
    ...
    "conversion": {
        ...
        "formula": "(a / b) * 100",
        "interpretation": "Конверсия показывает, какой процент посетителей становится клиентами.",
    ...
    "cpl": {
        ...
        "formula": "a / b",
        "interpretation": "CPL — сколько стоит привлечение одного потенциального клиента.",
...
SALES_CALCS = {
    ...
    "avg_check": {
        ...
        "formula": "a / b",
        "interpretation": "Средний чек — один из ключевых драйверов прибыли.",
    ...
    "pipeline": {
        ...
        "formula": "a * b / (c / 100)",
        "interpretation": "Показывает среднюю скорость прохождения сделок по воронке.",
...
LOGISTICS_CALCS = {
    ...
    "turnover_days": {
        ...
        "formula": "365 / a",
        "interpretation": "Сколько дней в среднем товар находится на складе.",
    ...
    "fill_rate": {
        ...
        "formula": "(a / b) * 100",
        "interpretation": "Показывает, какой процент заказов выполняется полностью.",
...
# ----- 5.7. ПРОИЗВОДСТВО - 5 калькуляторов -----
PRODUCTION_CALCS = {
    "oee": {
        ...
        "interpretation": "OEE показывает, насколько эффективно используется оборудование.",
        "advice": "1. Сократите простои\n2. Повысьте производительность\n3. Уменьшите брак",
        "problems": "❌ Частые простои оборудования\n❌ Много брака\n❌ Низкая производительность"
    },
    "defect_rate": {
        "name": "Процент брака",
        "inputs": ["Количество бракованных единиц", "Общее количество произведённых единиц"],
        "formula": "(a / b) * 100",
        "interpretation": "Какой процент продукции бракуется.",
        "advice": "1. Внедрите систему контроля качества\n2. Обучите персонал\n3. Обновите оборудование",
...
# ----- 5.8. IT И АВТОМАТИЗАЦИЯ - 4 калькулятора -----
IT_CALCS = {
    "automation_roi": {
        ...
        "advice": "1. Начинайте с малого\n2. Автоматизируйте сначала самые болезненные процессы\n3. Обучите персонал",
    ...
    "implementation_time": {
        ...
        "formula": "a / b",
        "interpretation": "Сколько месяцев займёт внедрение автоматизации.",
        "advice": "1. Правильно оценивайте объём\n2. Используйте Agile-методологию\n3. Вовлекайте бизнес-экспертов",
        "problems": "❌ Внедрение затягивается\n❌ Нет чёткого плана\n❌ Сотрудники не обучены"
    },
    "tco": {
        "name": "Совокупная стоимость владения (TCO)",
        "inputs": ["Стоимость внедрения (руб)", "Годовые затраты на поддержку (руб)", "Срок использования (лет)"],
        "formula": "a + b * c",
        ...
        "problems": "❌ Не учтены скрытые затраты\n❌ Система требует постоянных доработок\n❌ Нет бюджета на поддержку"
    },
    "digital_maturity": {
        "name": "Уровень цифровой зрелости",
        "inputs": ["Количество автоматизированных процессов", "Всего процессов"],
        "formula": "(a / b) * 100",
...
# ----- 5.9. УПРАВЛЕНИЕ ПРОЕКТАМИ - 5 калькуляторов -----
PROJECT_CALCS = {
    "spi": {
        "name": "Индекс выполнения сроков (SPI)",
        "inputs": ["Фактический объём работ", "Запланированный объём работ"],
        "formula": "a / b",
        "interpretation": "SPI больше 1 — проекты выполняются быстрее плана, меньше 1 — отстают.",
    ...
    "schedule_variance": {
        ...
        "advice": "1. Пересмотрите план\n2. Добавьте буфер времени\n3. Ускорьте критические задачи",
        "problems": "❌ Постоянные задержки\n❌ Нет буфера времени\n❌ Плохое планирование"
    }
}

# ----- 5.10. ЮРИДИЧЕСКИЙ ОТДЕЛ - 4 калькулятора -----
LEGAL_CALCS = {
    "contract_risk": {
        "name": "Юридические риски по договорам",
        "inputs": ["Количество спорных договоров", "Всего договоров"],
        "formula": "(a / b) * 100",
    ...
    "legal_claims": {
        ...
        "formula": "a / (b / 1000000)",
        "interpretation": "Сколько судебных дел приходится на 1 млн рублей выручки.",
    ...
    "compliance": {
        ...
        "inputs": ["Количество нарушений", "Количество проверок"],
        "formula": "100 - (a / b) * 100",
