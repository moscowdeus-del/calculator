...
def calculate_dashboard(dash_key, inputs):
    try:
        ...
        if dash_key == "financial_health":
            ...
            score = 0
            if revenue_per_employee > 5000000: score += 1
            if profit_per_employee > 500000: score += 1
            if opex_ratio < 30: score += 1
            if margin > 15: score += 1
            if revenue_per_client > 100000: score += 1
            ...
            return {
                ...
                "Выручка на сотрудника": f"{revenue_per_employee:,.0f} руб.",
                "Доля ФОТ": f"{payroll_ratio:.1f}%",
                "Текучесть": f"{turnover_rate:.1f}%",
                "Индекс зарплаты": f"{salary_index:.1f}%",
                "Оценка": rating
            }
...
def create_dashboard_pdf(results, chart_buf, user_info=None):
    ...
    try:
        ...
        story.append(Spacer(1, 20))
        story.append(Paragraph("📌 Отчёт сгенерирован автоматически.", 
...
Сайт: """ + SITE_LINK
    ...
    await context.bot.send_message(user_id, guide_text, parse_mode=None)
...
📅 {datetime.now().strftime('%d.%m.%Y %H:%M')}"""
    ...
    if data == "dash_chart":
        if not VISUALIZATION_AVAILABLE:
            await query.answer("❌ Визуализация недоступна", show_alert=True)
            return
        state, state_data = get_state(user_id)
        results = state_data.get("dash_results")
        if not results:
            await query.answer("❌ Сначала выполните расчёт!", show_alert=True)
            return
        try:
    ...
    if data == "dash_pdf":
        if not VISUALIZATION_AVAILABLE:
            await query.answer("❌ PDF недоступен", show_alert=True)
            return
        state, state_data = get_state(user_id)
        results = state_data.get("dash_results")
        if not results:
    ...
    if data.startswith("menu_"):
        ...
        if calc_list:
            set_state(user_id, "category", {"category": category, "page": 0})
            await query.edit_message_text(
                "Выберите калькулятор:",
                reply_markup=get_calc_list_keyboard(calc_list, page=0)
            )
        else:
            await query.answer("Калькуляторов пока нет.")
        return
    ...
    if data.startswith("page_"):
        ...
        calc_list = get_calc_groups().get(category, [])
        set_state(user_id, "category", {"category": category, "page": page})
        await query.edit_message_text(
            "Выберите калькулятор:",
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "dash_input":
        ...
        try:
            ...
            await update.message.reply_text("❌ Введите число:", reply_markup=get_calc_input_keyboard())
            return
        step += 1
        if step >= total:
    ...
    if state == "waiting_email":
        if re.match(r'^[^@]+@[^@]+\.[^@]+$', text):
            ...
            await send_guide(context, user_id)
        else:
            await update.message.reply_text("❌ Введите корректный email:")
        return

    await update.message.reply_text("Я не понимаю эту команду. Используйте /menu.")
